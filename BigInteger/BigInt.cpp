...
namespace bigint {
    ...
    BigInt &BigInt::operator+=(const BigInt &other) {
        ...
        if (this->sign == other.sign) {
            ...
            } else if (carry != 0 && max_object.digits.size() == min_object.digits.size())
                max_object.digits.push_back(carry);
        } else {
    ...
    BigInt &BigInt::operator^=(const BigInt &other) {
        ...
        std::string max_str = get_max_str(other);
        std::string min_str;
        if (max_str == (*this).Bigint_to_binary_str()) min_str  = other.Bigint_to_binary_str();
        else min_str = (*this).Bigint_to_binary_str();
    ...
    std::string BigInt::get_max_str(const BigInt &other) const {
